...
matchesurl = "https://www.sofascore.com/api/v1/search/events?q=" + hometeam + "%20" + awayteam + "&page=0"
...
homefull = fetch_and_parse_json("https://www.sofascore.com/api/v1/event/" + str(matchid))["event"]["homeTeam"]["name"]

awayfull = fetch_and_parse_json("https://www.sofascore.com/api/v1/event/" + str(matchid))["event"]["awayTeam"]["name"]
...
def match_list(seasonid, leagueid, round):
    ...
    for p in range(0, int(round)-1):
        ...
        for i in range(0, len(roundmatches)):
            ...
            awaygoalprobs.insert(0, y)
            full_list.append([str(hometeam)].append([most_likely(homegoalprobs, awaygoalprobs)[0], most_likely(homegoalprobs, awaygoalprobs)[1]]).append([str(awayteam)]))
            return full_list
...
def create_league_table(matches):
    ...
    # Create the table
    print(f"{'Team':<20} {'Points':<6} {'GD':<4} {'GS':<4} {'GP':<4}")
    for team, stats in sorted_teams:
        print(f"{team:<20} {stats['points']:<6} {stats['goal_difference']:<4} {stats['goals_scored']:<4} {stats['games_played']:<4}")
...
##create_league_table(matches)

lgid = leagueid()
ssid = seasonid(lgid)

print(match_list(ssid, lgid, 2))"""